...
def parse_args():
    ...
    parser.add_argument('--exp_setting', default='intra-subject', type=str, choices=['intra-subject', 'inter-subject'], help='Experiment setting: intra-subject or inter-subject (LOSO)')
    ...
    parser.add_argument('--rec', default = 'cat', type = str, help='Modal data concatenation mode')
    # Domain Encoder
    ...
    parser.add_argument('--mi_lr', default=1e-4, type=float,help='learning rate of MI_NET', dest='mi_lr')
    ...
    parser.add_argument('--seed', default=1024, type=int,help='seed for initializing training. ')
    ...
    parser.add_argument("--geo-loss", action = "store_true",  help = "Whether to use geometric loss")  
    parser.add_argument("--geo-loss-visual", action = "store_true",  help = "Whether to use Image geometric loss")
    parser.add_argument("--geo-loss-dis", type = str, default = 'cosine', help = "Distance in geo loss")
    parser.add_argument("--geo-last-epochs", type = int, default = 25, help = "Number of last epochs to use geometric loss")

    parser.add_argument("--base-alpha", type = float, default = 0.5, help = "Coeff of momentum update")
    parser.add_argument("--lambda1", type = float, default = 5.0, help = "Geometric Gaps loss hyperparameter lambda 1 (default 5.0)")
    #parser.add_argument("--lambda2", type = float, default = 0.5, help = "Geometric loss2 hyperparameter lambda 2")
    parser.add_argument("--lambda3", type = float, default = 1.0, help = "MI loss hyperparameter lambda 3")
    parser.add_argument("--lambda4", type = float, default = 1.0, help = "Reconstruct loss lambda 4")
